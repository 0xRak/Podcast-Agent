...
- **PDF Export**: Optional PDF generation with podcast-specific styling
...
**Advanced options**:
```bash
# Last 3 videos from past 14 days with PDF output
/podcast_summary @lexfridman --days=14 --limit=3 --pdf
...
1. **"wkhtmltopdf not found"** → Install system dependency
2. **"No transcript available"** → Video may not have captions  
3. **"Channel not found"** → Check channel handle spelling
4. **"Rate limit exceeded"** → Tool will automatically retry
5. **"Connection timeout"** → Check internet connection
